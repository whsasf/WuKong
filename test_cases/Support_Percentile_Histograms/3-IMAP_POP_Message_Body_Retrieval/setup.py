...
remote_operations.remote_operation(mx1_host1_ip,root_account,root_passwd,'su - {0} -c \'imconfcontrol -install -key \"/*/common/reportParamsInterval=30\";imconfcontrol -install -key \"/*/common/hostInfo=blobtier=Cassandra:blobcluster:9162\";imconfcontrol -install -key \"/*/mta/subAddressAllowedIPs=127.0.0.1\n10.49.58.239\n10.37.2.214\n10.6.105.42\";imconfcontrol -install -key \"/site1-inbound-standardmta-direct/mta/subAddressAllowedIPs=127.0.0.1\n10.49.58.239\n10.37.2.214\n10.6.105.42\"\''.format(mx_account),0)
basic_class.mylogger_record.info(' restart mss') 
remote_operations.remote_operation(mss1_host_ip,root_account,root_passwd,'su - {0} -c "~/lib/imservctrl killStart mss mta"'.format(mx_account),0)
remote_operations.remote_operation(mss2_host_ip,root_account,root_passwd,'su - {0} -c "~/lib/imservctrl killStart mss"'.format(mx_account),0)
time.sleep(10)  
...
for i in range(1,6):
    ...
    send_mail(mta1_host_ip,mta1_port,sender,[test_account_base+str(i)])
    ...
    send_mail(mta1_host_ip,mta1_port,sender,[test_account_base+str(i)+'+test'])
    ...
    time.sleep(2)
    basic_class.mylogger_record.info(' restart mss') 
    remote_operations.remote_operation(mss1_host_ip,root_account,root_passwd,'su - {0} -c "~/lib/imservctrl killStart mss"'.format(mx_account),0)
    remote_operations.remote_operation(mss2_host_ip,root_account,root_passwd,'su - {0} -c "~/lib/imservctrl killStart mss"'.format(mx_account),0)
